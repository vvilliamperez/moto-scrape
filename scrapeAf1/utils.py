...
def url_to_s3_path(url, prefix=None):
    ...
    logger.debug(f"Converted URL to S3 path: url: {url}, prefix: {prefix} -> {s3_friendly_path}")
...
def get_latest_2_object_from_s3(bucket_name: str, subdirectory: str) -> (object, object):
    """
    Retrieve the latest object from a specific subdirectory in the S3 bucket based on the LastModified date.
    """
    try:
